...
# ---------------------------------------------------
with open("dory_results.json", "r") as f:
    data = json.load(f)
...
# ---------------------------------------------------
# MSM G1 and G2 Time vs Vector Length
# ---------------------------------------------------
for group, ylabel, filename in [
    ("msm_g1_ms", "MSM G1 Time (ms)", "msm_g1_time.png"),
    ("msm_g2_ms", "MSM G2 Time (ms)", "msm_g2_time.png"),
]:
    plt.figure(figsize=(8, 5))
...
# ---------------------------------------------------
# Pairing Comparison (constant per curve)
# ---------------------------------------------------
pair_df = (
...
for curve in df["curve"].unique():
    ...
    plt.xlabel("Vector Length (n)")
    plt.ylabel("Time (ms)")
    plt.title(f"Dory Performance Breakdown ({curve})")
